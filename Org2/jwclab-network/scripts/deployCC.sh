...
if [ "$CC_SRC_PATH" = "NA" ]; then
	...
	if [ "$CC_NAME" = "iot" ]; then
		echo $'\e[0;32m'chaincode-iot$'\e[0m' chaincode
		CC_SRC_PATH="../chaincode-iot"
...
chaincodeQuery() {
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
		...
		set -x
		peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function":"QueryRecordHistory","Args":["DANANG_1"]}' >&log.txt
		res=$?
		set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
