...
# Check if your have the Common Connection Profile (CCP) template
if [ ! -d "ccp-template" ]; then
  echo "ERROR! Common Connection Profile (CCP) template not found.."
  exit 1
fi
...
#=================================STEP9=================================
echo
echo "---------> Step9: Generate CCP of $1 Organization"
echo
...
P0PORT=7051
CAPORT=32769
PEERPEM=$1_peer_org_crypto/tlsca/tlsca.$1.jwclab.com-cert.pem
CAPEM=$1_peer_org_crypto/ca/ca.$1.jwclab.com-cert.pem

echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > $1_peer_org_crypto/connection-$1.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > $1_peer_org_crypto/connection-$1.yaml
