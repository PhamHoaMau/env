...
function    usage {
    echo  "Usage: ./deployCC.sh    install | instantiate | invoke | query "
...
export CC_CHANNEL_ID="jwclabchannel"
...
case $OPERATION in
    ...
    "query")
        ...
        peer chaincode query -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"Args":["QueryRecordHistory","DANANG_1","TEMPERATURE"]}'

        ;;
    
    "invoke")
        echo "Invoke save record"
        peer chaincode invoke -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"Args":["UpdateRecord","QUANGNAM_1","TEMPERATURE","^C","25.5","2020-07-22 17:04:05"]}'
        ;;
